...
const AccommodationDetailsScreen: React.FC = () => {
  const [details, setDetails] = useState<AccommodationItem[]>([]);
  ...
  useEffect(() => {
    const fetchAccommodationDetails = async () => {
      try {
        const q = query(collection(db, "accommodations"), where("accommodationType", "==", subTopic));
        const querySnapshot = await getDocs(q);
        const fetchedDetails: AccommodationItem[] = querySnapshot.docs.map((doc) => {
          const data = doc.data();
          return {
        ...
        // Initialize active image index for each accommodation
  ...
  if (!details || details.length === 0) {
    return (
      ...
      <View style={styles.noDetailsContainer}>
        <Text style={styles.noDetailsText}>No accommodations available for {subTopic}.</Text>
      </View>
      </View>
              ...
              <View style={styles.infoContainer}>
                <View style={styles.infoRow}>
                  ...
                  <Text style={styles.text}>
                    LKR {item.pricePerNight} / ${convertToUSD(Number(item.pricePerNight))}
                  </Text>
                </View>
                <View style={styles.infoRow}>
